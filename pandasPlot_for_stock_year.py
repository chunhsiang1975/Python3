...
def convertDate(date):
  realyear=str(int(date)+1911)
  realdate=realyear
  return realdate


file_path = 'FMNPTK_1101_utf-8.csv'
...
sheet.delete_rows(idx=records_number+3,amount=7)
...
#Method 1
data = [
    Scatter(x=df['年度'], y=df['收盤平均價'], name='收盤平均價',mode='lines+markers'),
    Scatter(x=df['年度'], y=df['最低價'], name='最低價',mode='lines+markers'),
    Scatter(x=df['年度'], y=df['最高價'], name='最高價',mode='lines+markers')
]
...
#print(df['日期'])
df.plot(kind='line', figsize=(12, 6), x='年度', y=['收盤平均價', '最低價', '最高價'])  #繪製統計圖
